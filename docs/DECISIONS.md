...
| **DEC-33** | **RESUELTA.** Ni el DER ni la documentación funcional definen las **columnas del archivo**. El contrato canónico se fija en `apps.massive_load.parsers` (`identification_number`, `first_name`, `last_name`, `pathology` obligatorias; resto opcionales), alineado con los snapshots de `IMPORT_BATCH_ROW`. Como los archivos reales vendrán exportados de otro sistema, **el mapeo de cabeceras es configurable** con `MASSIVE_LOAD_COLUMN_ALIASES`: adaptarse a ellos no exige tocar código. Vacío por defecto — traducir al castellano en el código sería otra suposición encima de la primera. Un alias con destino inexistente **impide arrancar**, y si faltan columnas el error dice cuáles traía el archivo. Ya no requiere confirmación: no hay ninguna decisión atrapada en el código. |
